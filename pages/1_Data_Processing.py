...
def save_dataframe(cleaned_data, filename, filetype):                                                                          
    ...
    if filetype.lower() == "csv":                                                       # Determine the file extension based on the selected filetype                                                                 
        return df.to_csv(index=False).encode('utf-8'), f"{filename}.csv", "text/csv"    # Save as CSV
    elif filetype.lower() == "excel":
                                                                                        # Save as Excel
        return df.to_excel(index=False).encode('utf-8'), f"{filename}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    elif filetype.lower() == "text":
                                                                                        # Save as Text
        return df.to_csv(sep='\t', index=False).encode('utf-8'), f"{filename}.txt", "text/plain"
    else:
        return None, None, None
    
def plot_scatter_with_outliers(original_df, cleaned_df, column_names):                  # Create a scatter plot for each column
    for column in column_names:
        fig0, aj = plt.subplots()                                                       # Creating a new figure and axes
                                                                                        # Plotting original data 
        aj.scatter(original_df.index, original_df[column], color='blue', label='Original Data')
        outliers = cleaned_df[column].isna()                                            # Plotting outliers, Highlight outliers in red
        aj.scatter(original_df.index[outliers], original_df[column][outliers], color='red', label='Outliers')
...
def display_raw(df, selected_x_column, selected_y_column, chart_type):                  # Function to display raw data
...
def display_processed(cleaned_data, selected_x_column_f, selected_y_column_f, chart_type_f, num_points):
    if selected_x_column_f == "index":
        ...
        if isinstance(x_axis, pd.DatetimeIndex):                                        # Check if x_axis is already datetime index
            x_axis_date_strings = x_axis.strftime('%Y-%m-%d')                           # Convert directly to string
        else:
            x_axis_date_strings = x_axis.astype(str)                                    # Convert other index types to string
    else:
...
def matlab_to_datetime(matlab_time):                                                    # Function to convert MATLAB serial date number to Python datetime object
    ...
    matlab_epoch = datetime.datetime.fromordinal(1) + datetime.timedelta(days=366) - datetime.timedelta(days=1)
    offset = matlab_epoch - python_epoch
                                                                                        # Convert MATLAB time to Python datetime
    python_datetime = datetime.datetime.fromordinal(int(matlab_time)) + datetime.timedelta(days=matlab_time % 1) - offset
...
def main():
    ...
    if df is not None:
        ...
        columns_list = df.columns.tolist()                                              # Read column and select from the uploaded file
        ...
        if toggle_smoothing:                                                            # Check the state of the toggle button
            ...
            smoothing_option = st.radio("Choose a smoothing method", ["Moving Average", "Savitzky-Golay filter"])

            if smoothing_option == "Moving Average":
                st.subheader("Smoothing Parameters")

                st.write("Window Size for Moving Average:")                             # Help icon and details for moving average window size
                st.info("ℹ️ Window size controls the number of data points to consider for each moving average calculation.")
                filter_length = st.slider(label="Window Size", min_value=1, max_value=100, value=10, step=1)
   
            elif smoothing_option == "Savitzky-Golay filter":
                st.subheader("Smoothing Parameters")
                                                                                        
                st.write("Filter Length for Smoothing:")                                # Help icon and details for filter length
                ...
                filter_length1 = st.slider(label="Filter Length", min_value=0, max_value=1000, value=500, step=25)
                                                                                        
                st.write("Order of Smoothing:")                                         # Help icon and details for order length
                ...
                order_length = st.slider(label="Order of Smoothing", min_value=1, max_value=10, value=2, step=1)
            st.divider()
        ...
        with right:
             st.button('Reset', help="Click here to reset the process",                 # Button to reset the output
                       on_click=set_state, args=[0])

                                                                                        
        if toggle_outlier == True and st.session_state.stage >= 1:                      # Logic to perform preprocessing (Outlier removal)
            ...
            if outliers_option in ["Z-Score Method"]:
                ...
                outlier_result = outlier_detection.detect_outliers_zscore_drop(selected_columns, threshold)
                ...
                outlier_result = outlier_detection.detect_outliers_iqr_drop(selected_columns, threshold)
                st.write("Cleaned Data:", outlier_result)
                outlier_result.to_csv('Cleaned_data.csv', index=False)
                cleaned_data = outlier_result
               
            elif outliers_option in ["Isolation Forest"]:
                outlier_result = outlier_detection.detect_outliers_isof_drop(selected_columns, contamination_rate)
                st.write("Cleaned Data:", outlier_result)
                outlier_result.to_csv('Cleaned_data.csv', index=False)
                cleaned_data = outlier_result

            if len(selected_columns) == 1:
                outlier_plots = plot_scatter_with_outliers(df, cleaned_data, selected_columns)
                st.pyplot(outlier_plots)
            elif len(selected_columns) > 1:
                with st.expander("See Outlier Result Data"):
                        y_col = st.selectbox('Column Name', selected_columns, help='Select the column name from drop down to plot.')
                        outlier_plots = plot_scatter_multi_with_outliers(df, cleaned_data, y_col)
                        st.pyplot(outlier_plots)
        ...
        if toggle_outlier == True and toggle_interpol == True and st.session_state.stage >= 1:
            ...
            interpolated_df = interpolation.interpolation(Cleaned_data, selected_columns, method=interpolations_option)
            ...
            if len(selected_columns) == 1:
                interpolation_plot = interpolation_plot.plot_interpolated(interpolated_df ,selected_columns)
                st.pyplot(interpolation_plot)
            elif len(selected_columns) > 1:
                with st.expander("See Interpolated Result Data"):
                        y_col_interpol = st.selectbox('Name', selected_columns, help='Select the column name from drop down to plot.')
                        interpolation_plot = interpolation_plot.plot_interpolated(interpolated_df ,y_col_interpol)
                        st.pyplot(interpolation_plot)
        ...
        if st.session_state.stage >= 1:                                                 # Download the result
            filename = st.text_input('File Name', on_change=set_state, args=[2])        # Specify the file name
            filetype = st.selectbox('File Type', options=["CSV", "Excel", "Text"])      # Select file type
            
            if st.session_state.stage >= 2:                                             # Rename the processed columns
                cleaned_data = rename_columns_with_prefix(cleaned_data, selected_columns)
                cleaned_data = pd.DataFrame(cleaned_data)
                if filename and filetype:
                    file_content, file_name_with_extension, mime_type = save_dataframe(cleaned_data, filename, filetype)
                    st.download_button("Download",                                      #save the file as per the filename and type
                        data=file_content,
