...
# custom imports
from MLmodel.classes import Classification
...
class TestClassification(unittest.TestCase):
    ...
    def test_data_types(self):
        ...
        self.assertIsInstance(
            self.classification_model.Y_train.values.ravel(), np.ndarray)
        self.assertEqual(
            self.classification_model.Y_train.values.ravel().ndim, 1)

        self.assertIsInstance(
            self.classification_model.Y_test.values.ravel(), np.ndarray)
        self.assertEqual(
            self.classification_model.Y_test.values.ravel().ndim, 1)

    def test_prediction_shape(self):
        # Check if the prediction has the correct shape. shape[0] return the data row number
        self.assertEqual(self.classification_model.prediction['train'].shape[0], len(
            self.classification_model.X_train))
        self.assertEqual(self.classification_model.prediction['test'].shape[0], len(
            self.classification_model.X_test))

    def test_evaluation(self):
        # Check if evaluation train and test results are dataframe
        self.assertIsInstance(
            self.classification_model.evaluation['train'], pd.DataFrame)
        self.assertIsInstance(
            self.classification_model.evaluation['test'], pd.DataFrame)
