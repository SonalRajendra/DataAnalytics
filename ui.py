...
def split_data(data_processor):
    # select test size
    test_size_input = st.slider("Select test size", 0.1, 1.0, 0.5)
    data_processor.splitData(test_size=test_size_input)
    return data_processor

# select model to perform the training
def select_model():
    model_option = st.selectbox("Select your AI model 🤖", 
...
def get_training_config(model_option):
    ...
    if model_option == 'Neural Network':
        ...
        with col1:
            selected_classifier_or_regressor = st.selectbox("Nature of Data: ",
                        ('Classification', 'Regression')
                        )
        with col2:
            act_fn = st.selectbox("Activation Function: ",
                        ('relu', 'tanh', 'logistic')
                        )
        with col3:
            no_of_layers = st.number_input('Number of hidden layers', min_value=10, max_value=100, step=5)
        with col4:
            no_of_neurons = st.number_input('Number of neurons', min_value=100, max_value=500, step=10)

        return NeuralNetworkModel(classifier_or_regressor=class_mapping_nn[selected_classifier_or_regressor], 
    ...
    # training configuration for Random Forest
        ...
        with col2:
            n_est = st.number_input('Number of Estimators', min_value=100, max_value=1000, step=50)

        with col3:
            criterion = st.selectbox("Criterion: ",
                        ('gini', 'entropy', 'log_loss')
                        )
        return RandomForestModel(classifier_or_regressor=class_mapping_rf[selected_classifier_or_regressor], 
...
def main():
    ...
    if file_path:
        ...
        #evaluation = Evaluation(y_test=data_processor_split.y_test, y_pred=model.prediction)
        #print(f"Accuracy Score: {evaluation.get_accuracy()}")
        #print(f"RMSE: {evaluation.get_rmse()}")
        #print(f"Precision: {evaluation.get_precision()}")
        ## if status:
        ##if st.checkbox('show evaluation'):
        #get_evaluation(model, data_processor)
