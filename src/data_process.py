...
class OutlierDetection:
    ...
    def detect_outliers_zscore_drop(self, column_names, threshold):
        ...
        for column in column_names:  # Iterate through specified column names
            ...
            cleaned_data_df.loc[z_score_outliers, column] = (
                np.nan
            )  # Replace outliers with NaN values

        return (
            cleaned_data_df  # Return the DataFrame with outliers replaced by NaN values
        )
    ...
    def detect_outliers_isof_drop(self, column_names, contamination_rate):
        ...
        for column_name in column_names:  # Iterate through specified column names
            ...
            isolation_forest = IsolationForest(
                max_samples=100, contamination=contamination_rate, random_state=42
            )
            isolation_forest_outliers = isolation_forest.fit_predict(
                column_data.values.reshape(-1, 1)
            )
            outliers_mask = isolation_forest_outliers == -1
            cleaned_data_df.loc[outliers_mask, column_name] = (
                np.nan
            )  # Replace outliers with NaN values
        return cleaned_data_df
...
class Smoothing:
    ...
    def moving_average(self, column_name, filter_length):
        ...
        for column in column_name:  # Iterating through specified column names
            ...
            df_var[column] = (
                self.df[column].rolling(filter_length, min_periods=1).mean()
            )
...
class Interpolation:
    def __init__(self, available_methods=["linear", "quadratic", "cubic"]):
        self.available_methods = (
            available_methods  # Initializing available interpolation methods
        )
    ...
    def interpolation(self, df, column_names, method="linear"):
        for column_name in column_names:  # Iterate through specified column names
            ...
            if (
                ...
                raise ValueError(
                    "Invalid interpolation method. Please choose one of the following: {}".format(
                        self.available_methods
                    )
                )
